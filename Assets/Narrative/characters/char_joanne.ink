    ...
    */
    ...
    = conversation
        {shuffle:
            ...
            - {not already_saw(->sportsball_pregameritual_cats): ->converse(->sportsball_pregameritual_cats)}
            - {not already_saw(->schooldance_bodyimage): ->converse(->schooldance_bodyimage)}
        }
...
// this is always your first convo with Joanne.
    = sportsball_game
        ...
        +{have_mask(flirty) and not has_unlock(joanne_unlocks, cats)} YOU: WINK.
        ...
        +{have_mask(angry) and not has_unlock(joanne_unlocks, bakery)} YOU: There's more to life than sportsball!
            JOANNE: Whoa, rude!
...
//randomly select one of these topics after sportsball_game, repeats OK due to deadline
    = sportsball_doyoulike
        ...
        +{have_mask(flirty) and not has_unlock(joanne_unlocks, body_image)} YOU: Only if <i>you're</i> playing.
            JOANNE: Oh, stop. 
    ...
    = sportsball_pregameritual_cats
        ...
        +{have_mask(brooding) and not has_unlock(joanne_unlocks, cats)} [YOU: Don't you just like, show up and play?]
        ...
        +{have_mask(flirty) and not has_unlock(joanne_unlocks, bakery)} [YOU: And I'm locked in on you. WINK.]
            ...
            ~get_unlock(joanne_unlocks, bakery)
            ~get_mask(angry)
        +{have_mask(bubbly) and not has_unlock(joanne_unlocks, body_image)} [YOU: Wow... do you, like, spend hours on your hair and makeup?]
            JOANNE: Please.
    ...
    = schooldance_bodyimage
        ...
        +{not has_unlock(joanne_unlocks, bakery)}[YOU: Aren't you going to the dance?]
        ...
        +{have_mask(flirty) and not has_unlock(joanne_unlocks, cats)} [YOU: If only a star sportsballer would ask me to the dance...]
            JOANNE: Cute. You know who I would take if I was going?
            JOANNE: <bold>My cat</bold>, Mr. Fluffles.
            JOANNE: He's a perfect gentleman. 
            ~get_unlock(joanne_unlocks, cats)
            ~get_mask(angry)
        +{have_mask(bubbly) and not has_unlock(joanne_unlocks, body_image)} [YOU: Dances are just so magical!]
            JOANNE: And awkward.
